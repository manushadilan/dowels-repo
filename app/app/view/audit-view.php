<?php 
session_start();
if(!isset($_SESSION['logged_in']) || $_SESSION['role']!="audit"){
...
// get the records from the database
	if ($result = $mysqli->query("SELECT * FROM payment WHERE Approved='0'"))
	{
		...
		{
		echo "<br><div class='text-center'><span id='feedback' class='alert alert-info' >No records to display.</span></div>";
		}
	}
...
?>
	<input type="submit" class="btn btn-info " value="APPROVE" name="submit" />
...
if(isset($_POST["submit"]))
{
	...
	if(!isset($_POST['num']))
	{
		echo "<br><div class='text-center'><span id='feedback' class='alert alert-info' >Please Select a Record.</span></div>";
	}else{
		$box=$_POST['num'];
		While(list($key,$val)=@each($box)){
			
			if($mysqli->query("UPDATE payment SET Approved='1' WHERE PayID=$val"))
			{
